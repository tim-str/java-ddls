...
# TODO: handling data pre-processing in the source extract for 'event_cards'
...
# archive-qa-ap-southeast-2-am_event_transfers-2022-01-05-01 -> cm-fdz_cloud-am_event_transfers-20220105010001-2
# archive-qa-ap-southeast-2-am_event_cards-2022-01-05-01 -> cm-fdz_cloud-am_event_cards-20220105010001-8
# ...
...
transform_name() {
  ...
  case ${1} in
  "archive-qa-ap-southeast-2-am_event_cards")
    ...
    ;;
  "archive-qa-ap-southeast-2-am_event_transfers")
    NEW_FILENAME="cm-fdz_cloud-am_event_transfers"
    echo ${NEW_FILENAME}$dash$minus_four$minus_three$minus_two$minus_one$filler$dash$LINES_NUM
    ;;
  "archive-qa-ap-southeast-2-am_event_status")
    NEW_FILENAME="cm-fdz_cloud-am_event_status"
    echo ${NEW_FILENAME}$dash$minus_four$minus_three$minus_two$minus_one$filler$dash$LINES_NUM
    ;;
  "archive-qa-ap-southeast-2-am_event_status_reason")
#    echo "setting up cloud table name as <cm-fdz_cloud-am_event_status_reason>"
    NEW_FILENAME="cm-fdz_cloud-am_event_status_reason"
    echo ${NEW_FILENAME}$dash$minus_four$minus_three$minus_two$minus_one$filler$dash$LINES_NUM
    ;;
  "archive-qa-ap-southeast-2-am_status_reason")
    NEW_FILENAME="cm-fdz_cloud-am_status_reason"
    echo ${NEW_FILENAME}$dash$minus_four$minus_three$minus_two$minus_one$filler$dash$LINES_NUM
    ;;
  "archive-qa-ap-southeast-2-fdz_app_mgdlist_item")
  NEW_FILENAME="cm-fdz_cloud-fdzapp_mgdlist_item"
  echo ${NEW_FILENAME}$dash$minus_four$minus_three$minus_two$minus_one$filler$dash$LINES_NUM
  ;;
  esac
}
...
#      echo "Start timestamp: $( date +%s )"
      ...
      case ${2} in
      "archive-qa-ap-southeast-2-fdz_app_mgdlist_item")
        sed -e 's/\("{\)\(""value"":""\)\([a-z]*\)\("",""\).*\(""}""\)\(,""CustomerAccountType"":""\)\([A-Za-z]*\)\("",""\)\(LegalAccountName"":""\)\([0-9A-Za-z]*\)\("",""\)\(ReasonForChange"":""\)\([0-9A-Za-z]*\)\(""}"\)/"\3"/g' $csv_filename > ${csv_filename}temp
        sed -e 's/\("{\)\(""value"":""\)\([a-z]*\)\(""}"\)/"\3"/g' ${csv_filename}temp > $DESTINATION
        rm ${csv_filename}temp >/dev/null
        rm ${csv_filename} >/dev/null
      ;;
      *)
